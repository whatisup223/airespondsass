...
<head>
    ...
    <title>@yield('title') - AutoReply Dashboard</title>
        ...
        <aside class="w-72 glass-sidebar flex-shrink-0 relative z-20 flex flex-col transition-all duration-300"
            ...
            <nav class="flex-grow px-4 space-y-2 overflow-y-auto">
                <a href="/user/dashboard"
                    class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all hover:bg-white/5 {{ request()->is('user/dashboard') ? 'active-link' : 'text-gray-400 hover:text-white' }}">
                    <svg class="w-5 h-5 {{ request()->is('user/dashboard') ? 'text-indigo-500' : '' }}" fill="none"
                ...
                <a href="/user/automation"
                    class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all hover:bg-white/5 {{ request()->is('user/automation') ? 'active-link' : 'text-gray-400 hover:text-white' }} group">
                    <svg class="w-5 h-5 {{ request()->is('user/automation') ? 'text-pink-500' : 'group-hover:text-pink-500' }}"
                ...
                <a href="/user/automation"
                    class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all hover:bg-white/5 {{ request()->is('user/automation') ? 'active-link' : 'text-gray-400 hover:text-white' }} group">
                    <svg class="w-5 h-5 {{ request()->is('user/automation') ? 'text-indigo-500' : 'group-hover:text-indigo-500' }}"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="/user/inbox"
                    class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all hover:bg-white/5 {{ request()->is('user/inbox') ? 'active-link' : 'text-gray-400 hover:text-white' }} group">
                    <svg class="w-5 h-5 {{ request()->is('user/inbox') ? 'text-pink-500' : '' }}" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="/user/accounts"
                    class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all hover:bg-white/5 {{ request()->is('user/accounts') ? 'active-link' : 'text-gray-400 hover:text-white' }}">
                    <svg class="w-5 h-5 {{ request()->is('user/accounts') ? 'text-blue-500' : '' }}" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
